...
| 105 | 2026-09-07 | `GitHub Copilot` | — | — | **~30 min (complexity-based estimate)** | Run #44 — ViewSubmissions pagination font colour + slowness fixed. Root-caused the colour mismatch: GOV.UK's current-page pagination item has a blue background with white link text via `:link`/`:visited` pseudo-classes, which never match the `<button>`-based POST pagination — fixed with a scoped `.govuk-pagination__item--current .app-link-button{color:#fff}` CSS override (also fixes the same latent bug on every other POST-paginated grid). Root-caused the slowness to `LoadLookupsAsync` awaiting 5 independent lookups sequentially plus the search query after that (6 sequential DB round trips per request) — parallelized with `Task.WhenAll`. Build 0 errors; `dotnet test` 190 passed, 1 skipped, 0 failed. |
---
