...
| 4 | `ArchiveBlocks.aspx` | [Archive/ArchiveBlocks.cshtml](../src/Histo.Web/Pages/Archive/ArchiveBlocks.cshtml) | ✅ | Rebuilt 2026-09-08: previously a read-only 3-column grid with no archive-write capability at all; now a real Sender Ref/Histology Ref/Block Ref/Archived Date/Archive Location grid with select+bulk-update workflow, matching legacy's `btnUpdate_Click` single-vs-bulk semantics and date-range validation. Also fixed `EditBlock`/`AddBlock` SP parameter mismatches discovered during this rebuild (see `BlockRepository.SaveAsync`). |
| 5 | `ArchiveMenu.aspx` | [Archive/ArchiveMenu.cshtml](../src/Histo.Web/Pages/Archive/ArchiveMenu.cshtml) | ✅ | |
| 6 | `ArchiveTissues.aspx` | [Archive/ArchiveTissues.cshtml](../src/Histo.Web/Pages/Archive/ArchiveTissues.cshtml) | ✅ | Rebuilt 2026-09-08: previously queried Animals (wrong entity — SenderRef/HistologyRef/OnHold, unrelated to archiving) with no write capability; now queries submission-owned Tissues (matching legacy's tissue-level granularity) with the same select+bulk-update archive workflow as ArchiveBlocks. Simplification: one row per tissue record, not per physical piece (legacy duplicates rows per `NoPieces`, which carries no archive-relevant information). |
...
| 14 | `BatchesForArchiving.aspx` | [Batches/BatchesForArchiving.cshtml](../src/Histo.Web/Pages/Batches/BatchesForArchiving.cshtml) | ✅ | Fixed 2026-09-08: per-row "Archive blocks"/"Archive tissues" links passed `batchId` in the URL, but neither target page bound/read it (only read stale `Session.BatchID`) — added `[BindProperty(SupportsGet=true)] BatchId` to `ArchiveBlocksModel`/`ArchiveTissuesModel`/`ArchiveMenuModel`, set on entry. |
