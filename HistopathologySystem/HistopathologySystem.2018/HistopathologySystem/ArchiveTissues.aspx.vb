...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        VLAHeader1.PageTitle = "Archive Tissues"
        CheckPermissions()
        ...
        If Not IsPostBack Then
            ...
            Else
                PromptBeforeSaveScript("Are you sure you want to Cancel? Any archive information entered since you last clicked the Done button will be lost.", btnCancel)
            End If
...
#Region "Grid Handling"
    ...
    Private Sub InitialiseTissuesGrid()
        Try
            ...
            Dim dtArchivedTissuesList As New DataTable
            Dim dtHistologyRefList As New DataTable

            If Not dsBatchDetails Is Nothing Then
                If Not objSummary.CreateArchiveTissueSummaryData(dsBatchDetails, dtSummary, dtTissuesList, dtArchivedTissuesList, dtHistologyRefList) Then
                    Throw New Exception("BatchSummary.CreateArchiveTissueSummaryData returned false.")
                End If
            End If
            ...
            Session.Item(SessionVars.SV_TissuesTable) = dtSummary
            Session.Item(SessionVars.SV_TissuesView) = dv

            grdTissues.DataSource = dtSummary
            ...
            TissuesPager.AllowAddNew = False
            TissuesPager.AllowEdit = False
            TissuesPager.AllowDelete = False
            TissuesPager.Rebind()
            TissuesPager.Refresh()

            'Setup the filter dropdown lists
            If Not dtArchivedTissuesList Is Nothing Then
                ddlTissueList.DataSource = dtArchivedTissuesList
                ddlTissueList.DataValueField = "ID"
                ddlTissueList.DataTextField = "Description"
                ddlTissueList.DataBind()
                AddItemToDropDownList(ddlTissueList, "")
            End If
        ...
        Catch ex As Exception
            clsAppError.DisplayError("Failed to Initialise the Tissues Archive grid.", ex)
        End Try
    End Sub
#End Region
...
#Region "Lookup Lists"
    ...
    Private Sub LoadLookupLists()
        Try
            ...
            End If
        Catch ex As Exception
            clsAppError.DisplayError("Failed to load the lookup lists, Archive Tissues page.", ex)
        End Try
    End Sub
...
#Region "Event Handlers"
        ...
        Try
            ...
            If ValidatePageNumberFields() Then
                ...
                If iPageNumber > 0 Then
                    grdTissues.CurrentPageIndex = iPageNumber - 1
                Else
    ...
    Private Sub VLAHeader1_HomeClick(ByVal sender As Object, ByVal e As HistopathologySystem.HomeLinkEventArgs) Handles VLAHeader1.HomeClick
        If CType(Session.Item(SessionVars.SV_ViewSubmission), Boolean) = False Then
            ...
            sMessage.Append("You are currently entering tissue archive information. Any data that you have entered since you last saved will be lost. Are you sure you wish to exit?")
    ...
    Public Sub Check_Clicked(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
            If Not CType(Session.Item(SessionVars.SV_ViewSubmission), Boolean) = True Then
                ...
                Dim dtData As DataTable = CType(Session.Item(SessionVars.SV_TissuesTable), DataTable)
                ...
                For Each dgItem In grdTissues.Items
                    chkSelected = dgItem.FindControl("cbSelected")

                    iIndex = dgItem.ItemIndex
                    grdTissues.SelectedIndex = iIndex

                    iID = Convert.ToInt32(grdTissues.DataKeys(grdTissues.SelectedIndex))
                    sFilter = "ID = " & CStr(iID)
                ...
                If iCount = 1 Then
                    ...
                    For Each dgItem In grdTissues.Items
                        ...
                        If Not chkSelected Is Nothing Then
                            If chkSelected.Checked = True Then
                                grdTissues.SelectedIndex = dgItem.ItemIndex
                                Exit For
                            End If
                        End If
                    Next
    ...
    Private Sub cbSelectAll_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbSelectAll.CheckedChanged
        Try
            ...
            Dim drRow As DataRow
            Dim dtData As DataTable = CType(Session.Item(SessionVars.SV_TissuesTable), DataTable)
            Dim dv As DataView = CType(Session(SessionVars.SV_TissuesView), DataView)
            ...
            btnUpdateSelected.Enabled = cbSelectAll.Checked
    ...
    Private Sub btnUpdateSelected_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdateSelected.Click
        Try
            ...
            If ValidateMandatoryFields() Then
                If Not ctlArchiveDate.DateField = "" Then
                    lblError.Visible = False
                    If ctlArchiveDate.Validate(dDate, CDate(dReceivedDate.ToShortDateString), ctlArchiveDate.ValidationType.eValidateEarliest, "Archive date must be the same or later than the Submission received date of " & dReceivedDate.ToShortDateString) And _
                       ctlArchiveDate.Validate(dDate, CDate(dDate.Now.ToShortDateString), ctlArchiveDate.ValidationType.eValidateLatest, "Must be today or earlier") Then
                        Dim drData() As DataRow
                        Dim dtData As DataTable = CType(Session.Item(SessionVars.SV_TissuesTable), DataTable)
                        Dim dgItem As DataGridItem
            ...
            ctlDIV.InnerHtml = ""
        Catch ex As Exception
            clsAppError.DisplayError("Failed to update the selected records with the tissue archive information.", ex)
        End Try
    End Sub

    Private Sub grdTissues_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles grdTissues.SelectedIndexChanged
        Try
            Dim drData As DataRow()
            Dim dtData As DataTable = CType(Session.Item(SessionVars.SV_TissuesTable), DataTable)
            Dim dgItem As DataGridItem = grdTissues.SelectedItem
            Dim iID As Int32 = Convert.ToInt32(grdTissues.DataKeys(grdTissues.SelectedIndex))
        ...
        Catch ex As Exception
            clsAppError.DisplayError("Failed to display archive details for selected item.", ex)
        End Try
    End Sub
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Dim bSuccess As Boolean = objBatch.UpdateBatchDetails(CType(Session(SessionVars.SV_HeaderUserID), Integer), dsBatchDetails, objErrorlist, False, iBatchID)
        If bSuccess Then
            ...
            End If
        Else
            ctlDIV.InnerHtml = "<p><font color=""Red"">The database has not been updated because the following error(s) occurred:</p><p>&nbsp;</color></p><p><font color=""Red"">" & Join(objErrorlist.ToArray, "</color></p><p>") & "</p>"
        End If
    ...
    Private Sub rblFilter_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rblFilter.SelectedIndexChanged
        Try
            If rblFilter.SelectedItem.Value = "All" Then
                Dim dtData As DataTable = CType(Session.Item(SessionVars.SV_TissuesTable), DataTable)
                Dim dv As DataView = CType(Session.Item(SessionVars.SV_TissuesView), DataView)
                Dim sFilter As String
                ...
                SelectItemInDropDownList(ddlTissueList, "")
            ...
            End If
        Catch ex As Exception
            clsAppError.DisplayError("Failed to filter the tissue archive information.", ex)
        End Try
    End Sub
    ...
    Private Sub btnFilter_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFilter.Click
        Try
            ...
            If Not dv Is Nothing AndAlso Not dtData Is Nothing Then
                ...
                sTest = "TissueCode=" & "'" & ddlTissueList.SelectedItem.Text() & "'"

                If ddlHistologyRefList.SelectedItem.Value <> "" Then
                    sFilter = sHistoRef
                    If ddlTissueList.SelectedItem.Value <> "" Then
        ...
        Catch ex As Exception
            clsAppError.DisplayError("Failed to filter the archive tissue information.", ex)
        End Try
    End Sub
    ...
    Private Sub grdTissues_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles grdTissues.ItemDataBound
        ...
        Catch ex As Exception
            clsAppError.DisplayError("Failed to bind template columns in the archive tissues grid", ex)
        End Try
    End Sub
...
#Region "Private Functions"
    ...
    Private Sub SetEnterKeyPress()
        SetFocus(txtPage)
        SetDropDownControlOnEnter(ddlHistologyRefList, ddlTissueList.ClientID)
        SetDropDownControlOnEnter(ddlTissueList, btnFilter.ClientID)
        SetDropDownControlOnEnter(ddlArchiveLocation, ctlArchiveDate.FirstClientID)
        ctlArchiveDate.SetControlOnEnter(txtComment.ClientID)
    ...
    Private Sub UpdateSessionWithArchiveData()
        Try
            Dim dtArchiveData As DataTable = CType(Session.Item(SessionVars.SV_TissuesTable), DataTable)
            Dim dsBatchDetails As DataSet = CType(Session.Item(SessionVars.SV_BatchDetails), DataSet)
            Dim dtBatchTissues As DataTable = dsBatchDetails.Tables(HistopathologyLib.clsBatch.BATCH_TISSUES_TABLE)
            Dim drArchiveRow As DataRow
            Dim foundRow As DataRow()
            Dim sFilter As String

            'Copy the data from the datatable bound grid to the Session dataset
            If Not dtBatchTissues Is Nothing Then
                For Each drArchiveRow In dtArchiveData.Rows

                    sFilter = "ID=" & drArchiveRow("TissueID")
                    foundRow = dtBatchTissues.Select(sFilter)
        ...
        Catch ex As Exception
            clsAppError.DisplayError("Failed to retrieve Save the tissue archive data information.", ex)
        End Try
    End Sub
